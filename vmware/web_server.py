...
class openHandler(tornado.web.RequestHandler):
    ...
    async def post(self):
        ...
        # print(post_data)
        # if post_data["usrID"] not in openHandler.password:
        #     return 0
        if post_data["usrID"] not in openHandler.password:
            openHandler.password[post_data["usrID"]] = 1
            post_data["exec"] = "start1"
        else:
            post_data["exec"] = "start2"
...
class exitHandler(tornado.web.RequestHandler):
    ...
    async def post(self):
        ...
        if post_data["exec"] == "exit1":
            ...
            del docker_websocketHandler.clients[post_data["usrID"]]
            ws.close()
            del openHandler.password[post_data["usrID"]]
        else:
            send_data = {"error": "no such execution"}
        self.write(json.dumps(send_data))
